...
class CommentVoter extends TavroVoter
{
    ...
    protected function voteOnAttribute($attribute, $subject, TokenInterface $token)
    {
        ...
        if (!$user instanceof User) {
            // the active User must be logged in; if not, deny access
            return FALSE;
        }
     ...
     * @return bool
     */
    private function canView(Comment $comment, TokenInterface $token)
    {
        return TRUE;
    }
     ...
     * @return bool
     */
    private function canCreate(Comment $comment, TokenInterface $token)
    {
     ...
     * @return bool
     */
    private function canPatch(Comment $comment, TokenInterface $token)
    {
        if ($this->decisionManager->decide($token, array('ROLE_ADMIN'))) {
            return TRUE;
        }
