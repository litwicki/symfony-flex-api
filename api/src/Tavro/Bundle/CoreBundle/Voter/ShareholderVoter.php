 ...
 */
class ShareholderVoter implements VoterInterface, ContainerAwareInterface
{
    ...
    public function checkAccess($user, Shareholder $entity, $attribute)
    {
        ...
        if($attribute == self::EDIT || $attribute == self::PATCH) {
            ...
            if($user->getId() === $entity->getUser()->getId()) {
                ...
                /**
                 * Only allow the "author" to edit their Shareholder within 30 minutes of Shareholdering
                 */
                if($now < $modifyDate) {
                    return VoterInterface::ACCESS_GRANTED;
                }
